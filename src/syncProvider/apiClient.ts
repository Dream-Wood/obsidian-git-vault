...
/**
 * Returns true when the error was produced by `createApiError` and carries
 * the given HTTP status code.  Used to identify "empty repository" responses
 * (409 on GitHub, 404 on GitLab/Gitea) without parsing error message text.
 */
function hasApiStatus(err: unknown, status: number): boolean {
...
export class GitHubForgeClient extends ApiClientBase {
    ...
    async listRemoteFiles(): Promise<Map<string, ApiRemoteItem>> {
        let branchState: { commitSha: string; treeSha: string };
        try {
            branchState = await this.getBranchState();
        } catch (err) {
            // 409 = "Git Repository is empty." — treat as zero remote files.
            if (hasApiStatus(err, 409)) {
                return new Map();
            }
            throw err;
        }
        const tree = await this.requestJson<GitHubTreeResponse>(
            "GET",
            `/repos/${this.cfg.owner}/${this.cfg.repo}/git/trees/${branchState.treeSha}?recursive=1`
        );
        if (tree.truncated) {
            throw new Error(
        ...
        try {
            ...
            if (res.length > bytes) {
                if (
                    ...
                    console.debug(
                        "[Git Vault] downloadFileHeader: server returned more bytes than requested, treating as no-range support",
                        {
                            returned: res.length,
                            requested: bytes,
                            path: contentsPath,
    ...
    async commitMutations(
        ...
        // The GitHub Git Data API (blobs, trees, commits) returns 409
        // "Git Repository is empty." for ALL endpoints when the repo has zero
        // commits.  Check this BEFORE attempting blob creation.  If the repo
        // is empty we fall back to the Contents API (PUT /contents/{path}),
        // which creates one commit per file but works on brand-new repos.
        let isEmptyRepo = false;
        try {
            await this.getBranchState();
        } catch (err) {
            if (hasApiStatus(err, 409)) {
                isEmptyRepo = true;
            } else {
        ...
        if (isEmptyRepo) {
            // Contents API — sequential to avoid race-condition on branch creation
            // (GitHub creates the branch on the very first PUT).
            const createLimit = createConcurrencyLimiter(1);
            let count = 0;

            // Build a deterministic list of files we will create so we can
            // consistently label each commit with an index/total. Avoids
            // shared mutable counters captured by async closures.
            const filesToCreate = mutations.filter((m) => m.kind !== "delete");
            const totalFiles = filesToCreate.length;

            await Promise.all(
                filesToCreate.map((mutation, idx) =>
                    createLimit(async () => {
                        const b64 = Buffer.from(
                            toBytes(mutation.content)
                        ).toString("base64");
                        const perMessage = `${message} (initial sync ${idx + 1}/${totalFiles})`;
                        await this.requestJson(
                            "PUT",
                            `/repos/${this.cfg.owner}/${this.cfg.repo}/contents/${this.encodePath(mutation.path)}`,
                            {
                                message: perMessage,
                                content: b64,
...
export class GitLabForgeClient extends ApiClientBase {
    ...
    async listRemoteFiles(): Promise<Map<string, ApiRemoteItem>> {
        ...
        while (true) {
            ...
            try {
                items = await this.requestJson<GitLabTreeItem[]>(
                    "GET",
                    `/projects/${this.projectId}/repository/tree?ref=${encodeURIComponent(this.cfg.branch)}&recursive=true&per_page=1000&page=${page}`
                );
            } catch (err) {
                // 404 = branch/project doesn't exist yet (empty repo).
                // Only treat 404 as "empty repo" on the first page; if a
                // mid-pagination page returns 404 something unexpected
                // happened and we should not silently swallow it.
                if (hasApiStatus(err, 404)) {
                    if (page === 1) {
                        break;
                    }
                    // Preserve context to aid debugging before rethrowing.
                    // Pass project and page as structured data arguments rather
                    // than interpolating them into the format string. This
                    // prevents log-injection if projectId contains newlines or
                    // other control characters.
                    console.warn(
                        "[GitLabForgeClient] 404 listing tree (unexpected mid-pagination)",
                        { project: this.projectId, page },
                        err
                    );
                    throw err;
                }
...
export class GiteaForgeClient extends ApiClientBase {
    ...
    private async listTree(): Promise<Map<string, ApiRemoteItem>> {
        ...
        while (pendingDirectories.length > 0) {
            ...
            let responses: GiteaContentItem[][];
            try {
                ...
                // 404 on the root path means the repo is empty.
                // Only swallow 404 when the *resolved* contents path is the
                // repository root to avoid hiding 404s for missing
                // subdirectories. Compare resolved contents paths rather
                // than relying solely on the raw `path` string.
                if (hasApiStatus(err, 404)) {
                    const rootContents = this.contentsPath("");
                    if (
                        batch.length === 1 &&
                        this.contentsPath(batch[0]) === rootContents
                    ) {
                        return result;
                    }
                }
